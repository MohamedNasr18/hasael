...
import { useGetFarmInvestments, useRespondToInvestment, getGetFarmInvestmentsQueryKey } from "@workspace/api-client-react";
import { Layout } from "@/components/layout";
import { Card, CardContent } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Banknote, User as UserIcon, CheckCircle2, XCircle, Clock, Sprout } from "lucide-react";
...
export default function FarmInvestments() {
  ...
  const handleResponse = async (investmentId: number, action: InvestmentResponseInputAction) => {
    try {
      ...
      toast({
        ...
        description: action === "accepted" ? "تم قبول طلب الاستثمار." : "تم رفض طلب الاستثمار.",
      });
    } catch (error: any) {
      toast({
        variant: "destructive",
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case 'accepted': return <CheckCircle2 className="w-4 h-4 text-chart-2" />;
    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">طلبات الاستثمار</h1>
            <p className="text-muted-foreground mt-1 flex items-center gap-2">
              <Link href={`/farms/${farmId}`} className="text-primary hover:underline font-medium">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 gap-6">
            {investments.map((inv) => (
              <Card key={inv.id} className="bg-card border-border/50">
                  ...
                  <div className="flex flex-col lg:flex-row gap-6 justify-between">
                    <div className="space-y-4 flex-1">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <Link href={`/profile/${inv.investor_id}`} className="font-semibold hover:text-primary transition-colors">
                            {inv.investor_name || "مستثمر غير معروف"}
                          </Link>
                          <p className="text-sm text-muted-foreground">طلب في {format(new Date(inv.created_at), 'd MMM yyyy', { locale: ar })}</p>
                        </div>
                      </div>

                      <div className="flex gap-4">
                        <Badge variant="outline" className="flex items-center gap-1.5">
                          {inv.type === 'express_interest' ? <Sprout className="w-3.5 h-3.5" /> : <Banknote className="w-3.5 h-3.5" />}
                          {typeLabel(inv.type)}
                        </Badge>
                        <Badge className={`flex items-center gap-1 ${getStatusColor(inv.status)}`}>
                      ...
                      {inv.message && (
                        <div className="bg-secondary/50 p-4 rounded-lg border border-border/50">
                          <p className="text-sm italic text-muted-foreground">"{inv.message}"</p>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex flex-col lg:items-end justify-between min-w-[200px] border-t lg:border-t-0 lg:border-r border-border/50 pt-4 lg:pt-0 lg:pr-6">
                      <div className="mb-4 lg:mb-0 lg:text-right">
                        <p className="text-sm text-muted-foreground mb-1">المبلغ المقترح</p>
                        {inv.amount ? (
                          <p className="text-2xl font-bold text-primary">{inv.amount.toLocaleString()} ج.م</p>
                        ) : (
                          <p className="text-lg text-muted-foreground font-medium">غير محدد</p>
                        )}
                      </div>
          ...
          <div className="text-center py-20 bg-secondary/50 rounded-xl border border-dashed border-border/50">
            ...
            <p className="text-muted-foreground mt-2">لا توجد طلبات استثمار لهذه المزرعة حالياً.</p>
          </div>
        )}
      </div>
    </Layout>
